...
Both of these initialization files are called when the FreeCAD-gui is launched. Launching `freecadcmd` or importing FreeCAD (`import freecad`) will call the `__init__.py` file. (_The python import will not yet work on every system, but we are working towards a standardization_).
